...
def slider_input_with_label(label_text, slider_id, input_id, min_val, max_val, step, value, 
    ...
    return html.Div([
        ...
        dcc.Input(id=input_id, type='number', value=value, style={'marginTop': '10px', 'width': '100%'})
...
app.layout = dbc.Container([
    ...
    html.H2("Experimental Parameters", style={'color': 'orange'}),
        dbc.Row([
            dbc.Col([
                slider_input_with_label("Frequency (Low, Hz)", "freq-low-slider", "freq-low-input", 
                                        min_val=1, max_val=50, step=1, value=model.freq_low,
                                        marks={1:"1", 50:"50"}),
                slider_input_with_label("Frequency (High, Hz)", "freq-high-slider", "freq-high-input", 
                                        min_val=100, max_val=500, step=1, value=model.freq_high,
                                        marks={100:"100", 500:"500"}),
                slider_input_with_label("Charge at Low Frequency Unbound (µC)", "charge-low-unbound-slider", "charge-low-unbound-input",
                                        min_val=0, max_val=20, step=0.1, value=model.charge_low_unbound),
                slider_input_with_label("Charge at Low Frequency Bound (µC)", "charge-low-bound-slider", "charge-low-bound-input",
                                        min_val=0, max_val=20, step=0.1, value=model.charge_low_bound),
                slider_input_with_label("Charge at High Frequency Unbound (µC)", "charge-high-unbound-slider", "charge-high-unbound-input",
                                        min_val=0, max_val=20, step=0.1, value=model.charge_high_unbound),
                slider_input_with_label("Charge at High Frequency Bound (µC)", "charge-high-bound-slider", "charge-high-bound-input",
                                        min_val=0, max_val=20, step=0.1, value=model.charge_high_bound),
                slider_input_with_label("Electrode Area (mm²)", "electrode-area-slider", "electrode-area-input",
                                        min_val=1, max_val=100, step=0.1, value=model.electrode_area),
                slider_input_with_label("MB Electron Transfer (e⁻)", "mb-electron-slider", "mb-electron-input",
                                        min_val=1, max_val=5, step=1, value=model.mb_electron)
            ], width=6)
        ], style={'marginBottom': '20px'}),
...
###############################################################################
# Callbacks for Synchronizing Slider and Text Input for Experimental Params
###############################################################################
@app.callback(
...
# (Similarly create sync callbacks for charge and geometry parameters)
@app.callback(
    [Output("charge-low-unbound-slider", "value"), Output("charge-low-unbound-input", "value")],
    [Input("charge-low-unbound-slider", "value"), Input("charge-low-unbound-input", "value")]
)
...
@app.callback(
    [Output("charge-low-bound-slider", "value"), Output("charge-low-bound-input", "value")],
    [Input("charge-low-bound-slider", "value"), Input("charge-low-bound-input", "value")]
)
...
@app.callback(
    [Output("charge-high-unbound-slider", "value"), Output("charge-high-unbound-input", "value")],
    [Input("charge-high-unbound-slider", "value"), Input("charge-high-unbound-input", "value")]
)
...
@app.callback(
    [Output("charge-high-bound-slider", "value"), Output("charge-high-bound-input", "value")],
    [Input("charge-high-bound-slider", "value"), Input("charge-high-bound-input", "value")]
)
...
@app.callback(
    [Output("electrode-area-slider", "value"), Output("electrode-area-input", "value")],
    [Input("electrode-area-slider", "value"), Input("electrode-area-input", "value")]
)
...
@app.callback(
    [Output("mb-electron-slider", "value"), Output("mb-electron-input", "value")],
    [Input("mb-electron-slider", "value"), Input("mb-electron-input", "value")]
)
...
###############################################################################
# Main Callback (Including Experimental Parameters)
###############################################################################
@app.callback(
...
def update_plots(k_on, k_off, T_conc, I_min, I_max, hill_n,
    ...
    # Update model parameters from binding/SWV parts
    model.k_on = k_on
    model.k_off = k_off
    model.T_conc = T_conc
    model.I_min = I_min
    ...
    # SWV Frequency Response Plot (using our transformed method)
    inv_freqs, I_u_F, I_b_F, I_u_norm, I_b_norm, crossover_freq, crossover_inv_freq = model.compute_swv_curve()
    fig_swv = go.Figure()
    fig_swv.add_trace(go.Scatter(x=inv_freqs, y=I_u_norm, mode='lines', name='Unbound (Normalized)', line=dict(color='blue')))
    fig_swv.add_trace(go.Scatter(x=inv_freqs, y=I_b_norm, mode='lines', name='Bound (Normalized)', line=dict(color='red')))
    fig_swv.add_trace(go.Scatter(x=[crossover_inv_freq, crossover_inv_freq], y=[0, 1],
                                 mode='lines', name=f'Crossover = {crossover_inv_freq:.2f}', 
                                 line=dict(color='black', dash='dash')))
    fig_swv.update_layout(title='SWV Frequency Response',
                          xaxis_title='1 / Frequency (s)',
                          yaxis_title='Normalized (Peak Current)/Frequency',
                          template=plotly_template,
                          font_size=16)
    crossover_text = f"Crossover Frequency (raw): {crossover_freq:.1f} Hz"

    return fig_swv, crossover_text
