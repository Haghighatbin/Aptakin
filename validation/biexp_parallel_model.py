...
# ---------------------------------------------------------------------------------
# We only include one external stylesheet (light theme) because ThemeSwitchAIO will
# inject the appropriate Bootswatch CSS to switch the overall theme.
...
def double_exponential_swv_model(f, I0, k_et, f_decay1, f_decay2, A1):
    """
    Double-exponential SWV model:
      I_net(f) = I0 * exp(-k_et/(2f)) * [A1*exp(-f/f_decay1) + (1-A1)*exp(-f/f_decay2)]
    """
    return (I0 * np.exp(-k_et / (2.0 * f)) * (A1 * np.exp(-f / f_decay1) + (1.0 - A1) * np.exp(-f / f_decay2)))

def exp_swv_model(f, k_et, f_decay):
    """
    Returns the net current (simplified model) for a frequency f:
      I_net(f) = I0 * exp(-k_et/(2f)) * exp(-f/f_decay)
    I0 = 1.0 for simplicity. 
    """
    I0 = 1e-6
    return I0 * np.exp(-k_et / (2.0 * f)) * np.exp(-f / f_decay)

# ---------------------------------------------------------------------------------
# 3. Default Table Data (example)
#    Format: list of dicts, each dict has keys: Frequency, Unbound, Bound
# ---------------------------------------------------------------------------------
default_data = [
...
results_data = [
    ...
    {"Parameters": "A1",         "Unbound": 0.0, "Bound": 0.0},
...
app.layout = dbc.Container([
    ...
    # DataTable for user input. Note: initial styling is set for light mode.
    dash_table.DataTable(
        id='swv-table',
        columns=[
            {"name": "Frequency (Hz)", "id": "Frequency", "type": "numeric"},
            {"name": "Unbound (C)",     "id": "Unbound",   "type": "numeric"},
            {"name": "Bound (C)",      "id": "Bound",     "type": "numeric"},
        ],
        data=default_data,
        editable=True,
        row_deletable=True,
    ...
    # Graph for showing the data and fitted curves
    dcc.Graph(id='swv-graph', figure={}),
    dbc.Button("Fit Model", id="fit-button", color="primary", style={"marginBottom": 20, "marginTop": 20}),
    # Div for displaying the fitted parameters
    # html.Div(
    #     dcc.Textarea(
    #         id="fit-params",
    #         value='Textarea content initialised\nwith multiple lines of text',
    #         color="primary",
    ...
    dash_table.DataTable(
        id='results-table',
        columns=[
            {"name": "Parameters", "id": "Parameters", "type": "text"},
            {"name": "Unbound",     "id": "Unbound",   "type": "numeric"},
            {"name": "Bound",      "id": "Bound",     "type": "numeric"},
        ],
        data=results_data,
        editable=True,
        row_deletable=False,
...
# ---------------------------------------------------------------------------------
# 5. Callback: "Fit" button and theme switcher -> perform separate fits for unbound & bound -> update graph & text
# ---------------------------------------------------------------------------------
@app.callback(
...
def perform_fit(n_clicks, current_theme, table_data):
    ...
    try:
        ...
        popt_unbound, _ = curve_fit(double_exponential_swv_model,
                                                freqs,
                                                unbound_vals,
                                                p0=p0_unbound,
                                                bounds=bounds_u,
        ...
        I0_u, k_et_u, fd1_u, fd2_u, A1_u = popt_unbound

        # Print the optimised parameters
        print("Optimised parameters (UnBound):")
        param_names = ["I0", "k_et", "f_decay1", "f_decay2", "A1"]
        for name, val in zip(param_names, popt_unbound):
            print(f"  {name} = {val:.6g}")

        residuals = unbound_vals - double_exponential_swv_model(freqs, *popt_unbound)     # 1) Residuals
        ...
        print(e)
        I0_u, k_et_u, fd1_u, fd2_u, A1_u = (None,)*5    
    ...
    try:
        ...
        popt_bound, _ = curve_fit(double_exponential_swv_model,
                                                freqs,
                                                bound_vals,
                                                p0=p0_bound,
                                                bounds=bounds_b,
        ...
        I0_b, k_et_b, fd1_b, fd2_b, A1_b = popt_bound
        ...
        residuals = bound_vals - double_exponential_swv_model(freqs, *popt_bound)   # 1) Residuals
        ...
        print(e)
        I0_b, k_et_b, fd1_b, fd2_b, A1_b = (None,)*5
    ...
    # unbound fit
    if I0_u is not None:
        unbound_fit = double_exponential_swv_model(f_plot, I0_u, k_et_u, fd1_u, fd2_u, A1_u)
    else:
        unbound_fit = None
    
    # bound fit
    if I0_b is not None:
        bound_fit = double_exponential_swv_model(f_plot, I0_b, k_et_b, fd1_b, fd2_b, A1_b)
    else:
        bound_fit = None
    ...
    # f_extp = np.arange(10, 240 + step_size, step_size)
    f_extp = np.linspace(4, 1000, num_new_points)
    if I0_u is not None:
        unbound_fit_extp = double_exponential_swv_model(f_extp, I0_u, k_et_u, fd1_u, fd2_u, A1_u)
    else:
        unbound_fit_extp = None
    
    # bound fit
    if I0_b is not None:
        bound_fit_extp = double_exponential_swv_model(f_extp, I0_b, k_et_b, fd1_b, fd2_b, A1_b)
    else:
        bound_fit_extp = None
    ...
    # ---------------------------------------------------------------------------------
    def fmt_params(label, p):
        if any(val is None for val in p):
            return f"{label} failed."
        I0_, k_, fd1_, fd2_, A_ = p
        return (
            f"{label}: I0={I0_:.3g}, k_et={k_:.3g}, "
            f"f_decay1={fd1_:.3g}, f_decay2={fd2_:.3g}, A1={A_:.3g}"
        )
    
    # unbound_str = fmt_params("Unbound Fit", (I0_u, k_et_u, fd1_u, fd2_u, A1_u))
    # bound_str   = fmt_params("Bound Fit",   (I0_b, k_et_b, fd1_b, fd2_b, A1_b))
    # param_msg   = unbound_str + " | " + bound_str
    if any(val is None for val in (I0_u, k_et_u, fd1_u, fd2_u, A1_u)):
        print('Failed to fit the Unbound-State!')
    
    if any(val is None for val in (I0_b, k_et_b, fd1_b, fd2_b, A1_b)):
    ...
    {"Parameters": "A1",         "Unbound": f'{A1_u:.3f}',                  "Bound": f'{A1_b:.3f}'},
